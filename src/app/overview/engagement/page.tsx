...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function loadEngagementData() {
      const response = await fetch("/api/overview/engagement");
      const data = (await response.json()) as EngagementData;
  ...
  return (
    <div className="border-t border-slate-200 bg-slate-50/35 px-4 pb-5 pt-4 dark:border-slate-800 dark:bg-slate-950/30 md:px-5">
      <div className="grid gap-4 xl:grid-cols-4">
        {(engagementData?.scoreCards ?? []).map((card) => {
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-3">
        {(engagementData?.insights ?? []).map((insight) => {
      ...
      <section className="mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20">
        <div className="flex flex-col gap-3 px-4 pt-4 md:flex-row md:items-center md:justify-between md:px-5">
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
            {(engagementData?.legends ?? []).map((item) => (
              <span
                key={item.label}
        ...
        <div className="relative h-[320px] px-3 pb-3 pt-4 md:px-5">
          {engagementData ? (
            <EngagementQualityChart data={engagementData.qualityChart} />
          ) : (
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20">
          ...
          <div className="mt-4 h-72">
            {engagementData ? (
              <FeatureAdoptionChart data={engagementData.featureAdoptionChart} />
            ) : (
              <div className="h-full animate-pulse rounded-lg bg-slate-100 dark:bg-slate-950" />
            )}
          </div>
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20">
          ...
          <div className="mt-4 h-72">
            {engagementData ? (
              <JourneyDepthChart data={engagementData.journeyDepthChart} />
            ) : (
              <div className="h-full animate-pulse rounded-lg bg-slate-100 dark:bg-slate-950" />
            )}
          </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20">
          ...
          <div className="mt-5 space-y-4">
            {(engagementData?.featureRows ?? []).map((feature) => (
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20">
          ...
          <div className="mt-5 space-y-3">
            {(engagementData?.activityRows ?? []).map((activity) => {
      ...
      <section className="mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] border-collapse text-sm">
            ...
            <tbody>
              {(engagementData?.cohorts ?? []).map((cohort) => (
                <tr
                  key={cohort.name}
